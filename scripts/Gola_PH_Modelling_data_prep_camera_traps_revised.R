...
#### script written by Filibert Heim, filibert.heim@rspb.org.uk or filibert.heim@posteo.de, in Feb 2025, revised in May 2025
...
excel_sheets('data/combined_PH_data2025_draft10.xlsx')
pres_cam <- read_excel(path = 'data/combined_PH_data2025_draft10.xlsx', sheet = 'combined_camera_data')
deploy_cam <- read_excel(path = 'data/combined_PH_data2025_draft10.xlsx', sheet = 'locations_cameras')
...
# CYCV camera trap data 2020
pres_cam %>% filter(Project == 'CYCV camera trap data 2020') 
deploy_cam %>% filter(Project == 'CYCV camera trap data 2020') # it should be possible to join the data via Placename and Camera ID 
...
# CYCV camera trap data 2020
deploy_cam <- deploy_cam %>% mutate(SiteID = if_else(Project == 'CYCV camera trap data 2020', Placename, SiteID))
pres_cam <- pres_cam %>% mutate(SiteID = if_else(Project == 'CYCV camera trap data 2020', Placename, SiteID))
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
##### 7. create visits from deployment periods  ##### this step is not relevant anymore for the data prep but for the plots and overviews further down
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# arrange plots into one massive plot 
library(ggpubr)
